...
const getCursosGratis = (req, res) => {
    const estado = req.params.estado
    cursos.find({ estado, gratis : true }, function (err, cursos) {
        if (err) {
            res.status(500).send({ message: err.message })
        } else {
            res.status(200).send(cursos)
...
const getCursosPagos = (req, res) => {
    const estado = req.params.estado
    cursos.find({ estado, gratis : false }, function (err, cursos) {
        if (err) {
            res.status(500).send({ message: err.message })
        } else {
            res.status(200).send(cursos)
...
const postCurso = (req, res) => {
    ...
    curso.save(function(err){
        if (err) {
            ...
            res.status(201).send({ message : "curso foi incluído com sucesso"})
        }
    })
}
...
const deleteCurso = (req, res) => {
    ...
    cursos.deleteMany({ id }, function(err){
        if (err) {
            ...
            res.status(200).send({ message : "curso foi removido com sucesso"})
        }
    })
}
...
const deleteCursosPorTurno = (req, res) => {
    ...
    cursos.deleteMany(parametros, function(err){
        if (err) {
            ...
            res.status(200).send({ message : "cursos foram removidos com sucesso"})
        }
    })
}
...
const putCurso = (req, res) => {
    ...
    cursos.updateMany({ id }, { $set : req.body }, { upsert : true }, function(err){
        if (err) {
            ...
            res.status(200).send({ message : "curso foi atualizado com sucesso"})
        }
    })
}
